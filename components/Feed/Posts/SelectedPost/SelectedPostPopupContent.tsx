...
const SelectedPostPopupContent: FC<Props> = ({
  id,
  userId,
  authorImg,
  name,
  ...
  const [data, loading, error] = useDocumentOnce(
    doc(firestore, "users", userId + "", "posts", id + "")
  );
  ...
  if (error) {
    return (
      <div
        className="text-center p-5 bg-white mt-5 rounded-xl shadow-md w-full  
...
export default SelectedPostPopupContent;
